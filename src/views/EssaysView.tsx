...
export function EssaysView({ project, onChange }: { project: NovelProject; onChange: (project: NovelProject) => void }) {
  ...
  useEffect(() => {
    if (editing) saveEditorDraft(ESSAY_DRAFT_SCOPE, editing.id, editing)
  }, [editing])

  function openEditor(essay: Essay) {
    setEditing(loadEditorDraft<Essay>(ESSAY_DRAFT_SCOPE, essay.id) ?? essay)
  }

  function closeEditor() {
    if (editing) clearEditorDraft(ESSAY_DRAFT_SCOPE, editing.id)
    setEditing(null)
  }
  ...
  function save() {
    ...
    clearEditorDraft(ESSAY_DRAFT_SCOPE, normalized.id)
  ...
  return <div className="view-stack">
    <section className="toolbar-row">
      ...
      <div className="toolbar-actions">
        ...
        <button className="primary-button" onClick={() => openEditor(createEssay())}><Plus size={16} /> 新建随笔</button>
      </div>
    </section>
    <section className="essay-board">
      {visible.map((essay, index) => <button className="essay-row" key={essay.id} onClick={() => openEditor(essay)}>
    ...
    {editing && <div className="drawer-backdrop" onMouseDown={closeEditor}>
      <aside className="editor-drawer essay-drawer" onMouseDown={(event) => event.stopPropagation()}>
        <header className="drawer-header"><div><span className="eyebrow">ESSAY EDITOR</span><h2>{editing.title}</h2></div><button className="icon-button" onClick={closeEditor}><X size={18} /></button></header>
        ...
        <footer className="drawer-footer"><button className="ghost-button" onClick={closeEditor}>取消</button><button className="primary-button" onClick={save}>保存随笔</button></footer>
      </aside>
    </div>}
  </div>
}
